...
- [ ] `OpenHandsExecutor` compiles diagnostic findings and modeling remediation guidelines into a system prompt for the agent.
- [ ] Prompt explicitly instructs the agent to address specific failure modes:
  ...
  - S3 Upload: push best model weights using `s3-weights-storage`.
- [ ] Server initiates background agent execution using `DockerWorkspace` upon `POST /v2/fix`.
- [ ] Agent iterates, executes `train.py`, logs metrics to MLflow, and terminates cleanly via the communication skill webhook.
- [ ] Integration test executes a full mock/real diagnostic-to-fix run on a benchmark dataset (e.g. Wisconsin breast cancer).
